// trends.js - Versión estable con conexión real a YouTube Data API
const API_KEY = 'AIzaSyDAQVkMZ_l73dK7pt9gaccYPn5L0vA3PGw'; // ← pega aquí tu API real de YouTube
const YT_BASE = 'https://www.googleapis.com/youtube/v3';

// ====== Funciones base ======
const $ = id => document.getElementById(id);
const setStatus = msg => { if ($('status')) $('status').textContent = msg; };
const showError = msg => { const e=$('err'); e.style.display='block'; e.textContent=msg; };
const clearError = ()=>{ const e=$('err'); e.style.display='none'; e.textContent=''; };
...
async function runGenerator({brand,summary}){
  clearError(); setStatus('Buscando tendencias...'); $('loader').style.display='block';
  try{
