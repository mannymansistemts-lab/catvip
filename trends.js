...
function makeHash(text){
  if(!text) return '';
  const t=text.replace(/^#/,'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9\s]/gi,'').trim().replace(/\s+/g,'');
  return t ? '#' + t : '';
}
...
function freqSorted(arr){const map={}; arr.forEach(x=>{if(x)map[x]=(map[x]||0)+1;}); return Object.keys(map).sort((a,b)=>map[b]-map[a]);}
...
function generateSuggestions({brand,campaign,summary,country,topTokens,topHours}){
  ...
  const resultHashes=['#catalogosdigitales','#modaybelleza'];
  const maxPer=7;
  for(const t of topTokens){if(resultHashes.length>=maxPer) break; const h=t.startsWith('#')?t:makeHash(t); if(!resultHashes.includes(h)&&h.length>1) resultHashes.push(h);}
  ...
  const studioTags=[`${brandClean} ${campaignClean}`.trim(),`${brandClean} ${year}`.trim()];
  topTokens.slice(0,12).forEach(t=>{const tClean=t.replace(/^#/,''); if(tClean&&!studioTags.includes(tClean)) studioTags.push(tClean);});
...
function renderTendencias(list){
  const ul=$('tendencias'); if(!ul) return; ul.innerHTML='';
  if(!list||!list.length){ul.innerHTML='<li>No hay tendencias</li>'; return;}
  for(const t of list.slice(0,12)){const li=document.createElement('li'); li.textContent=safeText(t); ul.appendChild(li);}
}

function renderResultado(sugg){
  const out=$('resultado'); if(!out) return;
...
async function runGenerator({brand,campaign,summary,country='MX'}){
  clearError(); setStatus('buscando en YouTube...');
  try{
    ...
    renderResultado(suggestions); setStatus('listo'); return suggestions;
  }catch(err){
    setStatus('error'); showError('Error al generar sugerencias: '+(err.message||err));
    const fallback=generateSuggestions({brand,campaign,summary,country,topTokens:[],topHours:[19,20]});
    renderResultado(fallback); return fallback;
  }
}
